...
def generate_figure(pier_data_dict, 
                          ...
                          year,event,abutment_data,recur,LTD,contract_sta,contract_elev
                          ):
    ...
    for station in scour_data_copy:
        try:
            ...
            # This is done to ensure that the lt_deg values are updated correctly
            #abutment_data = bridge_data[['Ground Station Offset','SDAB','SCAB']]
            #abutment_data.loc[left_index[0]:right_index[0], abutment_data[1]] = np.nan
            #abutment_data.loc[left_index[0], abutment_data[1]] = station[0][1]
            #abutment_data.loc[right_index[0], abutment_data[1]] = station[2][1]
        except Exception as e:
            pass
        """
    ...
    if recur==0:
        ...
        ax.plot(gl[:len(abutment_data[1])], abutment_data[1], color='#2E3033', linewidth=3,linestyle='-', label=f'Wave Scour - {event}')
        ...
        gl = ground_line['Offset Station']
        ax.plot(gl[:len(abutment_data[1])], abutment_data[1], color='#2E3033', linewidth=1,linestyle='--', label=f'Wave Scour - {event}')
...
# Create the polygon
    ...
    for y in y_ticks:
        plt.hlines(y=y,xmin = -2, xmax = 0, color='grey',linewidth=1)
